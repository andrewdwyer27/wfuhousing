...
const TimeSlotAdmin = () => {
  ...
  // Generate time slots by class year
  const generateTimeSlotsByYear = async () => {
    try {
      // Group users by class year
      const usersByYear = {};
      users.forEach(user => {
        if (user.classYear) {
          if (!usersByYear[user.classYear]) {
            usersByYear[user.classYear] = [];
          }
          usersByYear[user.classYear].push(user);
        }
      });
      
      // Sort years chronologically (seniors first)
      const years = Object.keys(usersByYear).sort();
      
      // Create time slots (1 hour each) starting tomorrow at 9 AM
      ...
      // Assign time slots by year
      for (const year of years) {
        const usersInYear = usersByYear[year];
        
        // For each user in this year
        for (const user of usersInYear) {
          ...
          // Increment start time by 1 hour for next user
          tomorrow.setHours(tomorrow.getHours() + 1);
          
          // Reset to 9 AM for next day if it's past 5 PM
      ...
      showSuccessMessage("Time slots generated successfully by class year");
    } catch (error) {
      console.error('Error generating time slots:', error);
      alert("Error generating time slots: " + error.message);
    }
  };
                    ...
                    <button
                      onClick={() => {
                        const startTime = prompt("Enter start time (YYYY-MM-DD HH:MM):", 
                          user.timeSlot ? new Date(user.timeSlot.startTime).toISOString().slice(0, 16) : "");
                        const endTime = prompt("Enter end time (YYYY-MM-DD HH:MM):", 
                          user.timeSlot ? new Date(user.timeSlot.endTime).toISOString().slice(0, 16) : "");
                        
                        if (startTime && endTime) {
                          setUserTimeSlot(user.uid, {
