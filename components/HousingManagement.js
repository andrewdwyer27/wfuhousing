...
const HousingManagement = () => {
    ...
    // Function to clear all occupants from a room
    const handleClearRoom = async (dormId, roomId) => {
        if (window.confirm('Are you sure you want to remove all occupants from this room?')) {
            try {
                ...
                if (occupants.length === 0) {
                    setError("This room already has no occupants");
                    return;
                }
                ...
                // Update all users who were in this room
                ...
                setError(`Error clearing room: ${err.message}`);
            }
        }
    };
                                                    ...
                                                    <tr key={room.id}>
                                                                ...
                                                                <div>
                                                                    <div className="space-y-1">
                                                                        {getOccupantDetails(room.occupants).map(occupant => (
                                                                            <div key={occupant.id}>
                                                                                <span className={`text-sm ${occupant.unknown ? 'text-orange-500' : 'text-gray-700'}`}>
                                                                                    {occupant.name}
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                            <div className="space-y-2">
                                                                ...
                                                                {room.occupants && room.occupants.length > 0 && (
                                                                    <button
                                                                        onClick={() => handleClearRoom(dorm.id, room.id)}
                                                                        className="text-red-600 hover:text-red-900 block"
                                                                        title="Remove all occupants"
                                                                    >
                                                                        Clear Room
                                                                    </button>
                                                                )}
